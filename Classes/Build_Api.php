...
class Build_Api {
    ...
    public function save_action() {
        ...
        if (!wp_verify_nonce($wpnonce, 'oxi_vc_tabs_settings')) :
            return new WP_REST_Request('Invalid URL', 422);
            die();
        endif;
    ...
    public function post_create_new() {
        ...
        if (!empty($this->styleid)) :
            ...
            if (array_key_exists('css', $newdata) && $newdata['css'] != '') :
                $old = true;
                $this->database->wpdb->query($this->database->wpdb->prepare("INSERT INTO {$this->database->parent_table} (name, style_name, rawdata, css) VALUES (%s, %s, %s, %s)", [$this->rawdata, $newdata['style_name'], $newdata['rawdata'], $newdata['css']]));
            else :
                $this->database->wpdb->query($this->database->wpdb->prepare("INSERT INTO {$this->database->parent_table} (name, style_name, rawdata) VALUES ( %s, %s, %s)", [$this->rawdata, $newdata['style_name'], $newdata['rawdata']]));
            endif;
            ...
            if ($redirect_id > 0) :
                if ($old == true) :
                    ...
                    foreach ($child as $value) {
                        $this->database->wpdb->query($this->database->wpdb->prepare("INSERT INTO {$this->database->child_table} (styleid, rawdata, title, files, css) VALUES (%d, %s, %s, %s, %s)", [$redirect_id, $value['rawdata'], $value['title'], $value['files'], $value['css']]));
                    }
                else :
    ...
    public function post_json_import($folder, $filename, $name = 'truee') {
        if (is_file($folder . $filename)) {
            ...
            $this->database->wpdb->query($this->database->wpdb->prepare("INSERT INTO {$this->database->parent_table} (name, style_name, rawdata) VALUES ( %s, %s, %s)", [$style['name'], $style['style_name'], $style['rawdata']]));
            $redirect_id = $this->database->wpdb->insert_id;
    ...
    public function post_shortcode_active() {
        ...
        $tabs = 'responsive-tabs';
        if ($id > 0) :
            $this->database->wpdb->query($this->database->wpdb->prepare("INSERT INTO {$this->database->import_table} (type, name) VALUES (%s, %s)", [$tabs, $id]));
        ...
        foreach ($params as $value) {
            if ((int) $value) :
                ...
                if (array_key_exists('title', $data)) :
                    $this->database->wpdb->query($this->database->wpdb->prepare("INSERT INTO {$this->database->child_table} (styleid, rawdata, title, files, css) VALUES (%d, %s, %s, %s, %s)", [$data['styleid'], $data['rawdata'], $data['title'], $data['files'], $data['css']]));
                else :
     ...
     * @return void
     */
    public function post_oxi_vc_tabs_permission() {
        if (!current_user_can('manage_options')) {
            return;
        }
        $rawdata = json_decode(stripslashes($this->rawdata), true);
        $value = sanitize_text_field($rawdata['value']);
        update_option('oxi_vc_tabs_permission', $value);
            ...
            if (false === $license_data->success) {
                ...
                switch ($license_data->error) {
                    ...
                    case 'item_name_mismatch':
                        ...
                        $message = __('This appears to be an invalid license key for %s.');
                        break;
